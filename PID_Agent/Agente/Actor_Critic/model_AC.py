...
class ActorNetwork(nn.Module):
    ...
    def __init__(
        ...
        # Capas compartidas (extracción de features)
        layers = []
        input_dim = state_dim
        ...
        # Cabeza para la media de la acción (en [-1, 1] via tanh)
        self.mu_head = nn.Sequential(
            nn.Linear(input_dim, action_dim),
            nn.Tanh()
        )

        # Cabeza para log_std (se clampea para estabilidad)
    ...
    def get_distribution(self, state: torch.Tensor):
        """Retorna la distribución Normal para muestrear acciones."""
    ...
    def sample_action(self, state: torch.Tensor):
        ...
        log_prob = dist.log_prob(action).sum(dim=-1)  # suma sobre dimensiones de acción
...
class CriticNetwork(nn.Module):
    def __init__(
        ...
        layers.append(nn.Linear(input_dim, 1))  # V(s) escalar
